create database `10322_Finding User Purchases`;

use `10322_Finding User Purchases`;
...
with cte as(select * from amazon_transactions
order by user_id,created_at),
cte1 as(
select user_id, min(created_at)over(partition by user_id) as first_purchase,created_at,
timestampdiff(day,min(created_at)over(partition by user_id),created_at)<7
and timestampdiff(day,min(created_at)over(partition by user_id),created_at)>=1
as
time_diff
from cte
