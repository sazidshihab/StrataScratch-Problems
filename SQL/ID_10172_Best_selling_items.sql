create database `ID_10172_best_selling_items`;
...
('540954', '22381', 'TOY TIDY PINK POLKADOT', 4, '2011-01-12', 2.10, 14606, 'United Kingdom'),
...
('537141', '22193', 'RED DINER WALL CLOCK', 1, '2010-12-05', 8.50, 15570, 'United Kingdom'),
...
('15', '22087', 'PAPER BUNTING WHITE LACE', 2, '2011-02-28', 2.55, NULL, 'United Kingdom');

with cte as(
select month(invoicedate)as `month`,description, sum((quantity)*(unitprice)) as total_paid  from online_retail
where quantity >0
group by month(invoicedate),description
order by month(invoicedate)),
cte1 as(
select `month`,description,total_paid   from (
select `month`,description,total_paid,row_number()over(partition by month order by total_paid desc) as rn from cte) as b
where rn=1)
select * from cte1
;