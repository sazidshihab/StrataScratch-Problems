create database `10299_Finding Updated Records`;

use `10299_Finding Updated Records`;
show tables;
...
insert into ms_employee_salary values 
...
with cte as(
select id, first_name, last_name, max(salary)over(partition by id) as salary ,
case when salary=max(salary)over(partition by id) then department_id end as department_id  
from ms_employee_salary 
)
select * from cte where department_id is not null;